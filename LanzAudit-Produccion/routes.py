...
def nmapScan():
    ...
    if request.method == 'POST':
        ...
        if success:
            ...
            flash('Escaneo fallido: ' + result.get("error", "Error desconocido"), 'danger')
            
        return render_template('scan/nmap-scan.html', result=result if success else None, target=target, subtype=subtype)
